program saxpy_mpi
  ...
  !print*,"Rank: ",irank,"Size: ",nranks

  ! Check to see that the global length is evenly divisible by the number of MPI ranks
  if (mod(len_global, nranks) .ne. 0) then
    if (irank .eq. 0) then
      write(*,'("The global array length, ",I5,", is not divisible by the number of ranks,",I5)'), len_global, nranks
  ...
  ! Scatter operands
  call MPI_Scatter( X_global, len_local, MPI_REAL4, &
                    X_local,  len_local, MPI_REAL4, &
                    0,       MPI_COMM_WORLD,  ierr  )

  call MPI_Scatter( Y_global, len_local, MPI_REAL4, &
                    Y_local,  len_local, MPI_REAL4, &
                    0,        MPI_COMM_WORLD,ierr   )
  ...
  ! Gather result
  call MPI_Gather( Y_local, len_local, MPI_REAL4, &
                   Y_global,len_local, MPI_REAL4, &
                   0,       MPI_COMM_WORLD, ierr  )
